...
function startBackgroundBatch() {
  if (globalForCollect.__kizukiBatchPromise) {
    return { started: false, running: true };
  }

  console.log("[collect] background batch started");
  globalForCollect.__kizukiBatchPromise = runBatch()
    .then((result) => {
      console.log(
    ...
    .finally(() => {
      globalForCollect.__kizukiBatchPromise = null;
    });

  return { started: true, running: false };
}

// 手動再収集トリガー。RSS取得+LLM要約+トレンドスコア再計算をまとめて実行する。
