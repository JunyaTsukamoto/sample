...
/**
 * 収集→要約の一連のバッチ処理を実行し、batch_runsテーブルに結果を記録する。
 */
export async function runBatch() {
  ...
  try {
    ...
    const status = collectResult.sourcesFailed > 0 && collectResult.sourcesOk === 0
      ? "failed"
      : "success";
    ...
    return { runId, collectResult, summarizeResult };
  } catch (err) {
    finish.run({
      id: runId,
      ts: nowIso(),
...
export function getLatestRuns(limit = 10) {
  return db
    .prepare("SELECT * FROM batch_runs ORDER BY id DESC LIMIT ?")
    .all(limit);
}
