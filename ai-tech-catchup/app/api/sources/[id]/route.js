...
export async function PUT(request, { params }) {
  ...
  const next = {
    ...
    track_type: body.trackType ?? existing.track_type,
    fetch_method: body.fetchMethod ?? existing.fetch_method,
    is_active:
      typeof body.isActive === "boolean" ? (body.isActive ? 1 : 0) : existing.is_active,
  };

  db.prepare(
    `UPDATE sources SET name=@name, url=@url, track_type=@track_type,
     fetch_method=@fetch_method, is_active=@is_active, updated_at=@ts WHERE id=@id`
