...
export default function StatusPage() {
  ...
  const counts = Object.fromEntries(
    data.statusCounts.map((c) => [c.status, c.c])
  );
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div>
        ...
        <p className="text-sm text-slate-500">直近のバッチ実行結果と記事の処理状況を確認できます。</p>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-lg divide-y divide-slate-100">
        <h2 className="p-3 text-sm font-semibold text-slate-700">実行履歴</h2>
        {data.runs.length === 0 ? (
